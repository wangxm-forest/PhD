...
#with(subset(d, species == species_list[1]), {plot(year, filledseeds, type = "n", # 'n' means no points initially
...
species_list1 <- unique(subset_data1$species)
with(subset(d, species == species_list1[1]), {
  plot(year, totalseed, type = "n", # 'n' means no points initially
       xlab = "Year", ylab = "Total Seeds in 10 Stands", 
       ylim = c(0, 100),
       xaxt = "n")
...
# Add smooth lines for each species
for (i in 1:length(species_list1)) {
  species_data <- subset(d, species == species_list1[i])
  smooth_line <- smooth.spline(species_data$year, species_data$totalseed)  # Apply smoothing
  lines(smooth_line, col = colors1[i], lwd = 2)  # Add the smoothed line with a width of 2
}
...
#       xlab = "Year", ylab = "Total Seeds in 10 Stands", #       ylim = c(0, 70),
#       xaxt = "n")
#})
#axis(1, at = 2009:2017, labels = 2009:2017, las = 2)  # 'las = 2' rotates the labels
